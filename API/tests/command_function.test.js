 ...
 * use the ./tests/db_handle file instead ./db_config (schedule for prod env)
 */
...
describe('mongodb command response and connexion', () => {
    ...
    it('create a command normally', async () => {
        ...
        expect(findCommand.user_id).toBe(commandMocked.user_id)
    })
    ...
    it('try to create a command but all fields are missing', async () => {
        let req = httpMock.createRequest({ body: { kind: null, restaurant: null, paymentMethod: null, date: null, products: null, price: null, currency: null, state: null, user_id: null } })
        let res = httpMock.createResponse()
        await command.createCommand(req, res)
        let data = res._getJSONData()
        let status = res._getStatusCode()
    ...
    it('get a command normally', async () => {
        ...
        expect(dataFind.data.user_id).toBe(commandMocked.user_id)
    ...
    it('set a command', async () => {
        ...
        expect(dataIssus.data.user_id).toBe("12")
    })

    it('set a command but don\'t exist', async () => {
        let _id = "Efzfdaqzadcvg23"
        let commandNotExist = new commandModel({ _id: _id, kind: "delivery", restaurant: "le mas des aromes", paymentMethod: "check", date: new Date("12/12/12"), product: ["salade caesar"], price: 23, currency: "Euro", state: "delivered", user_id: "12" })
